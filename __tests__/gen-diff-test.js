...
describe('Simle Tests', () => {
  ...
  it('#gen-diff JSON -> structure', () => {
    const actual = genDiff(path1json, path2json, { format: 'default' });
  ...
  it('#gen-diff YAML -> structure', () => {
    const actual = genDiff(path1yml, path2yml, { format: 'default' });
    const expectedFixturePath = path.join(__dirname, '__fixtures__', 'expected_default');
    const expected = fs.readFileSync(expectedFixturePath, 'utf-8');
    expect(actual).toEqual(expected);
  });
  ...
  it('#gen-diff INI -> structure', () => {
    const actual = genDiff(path1ini, path2ini, { format: 'default' });
    const expectedFixturePath = path.join(__dirname, '__fixtures__', 'expected_default');
    const expected = fs.readFileSync(expectedFixturePath, 'utf-8');
    expect(actual).toEqual(expected);
  });
...
describe('Complex Tests', () => {
  const path1json = path.join(__dirname, '__fixtures__', 'before_complex.json');
  const path2json = path.join(__dirname, '__fixtures__', 'after_complex.json');

  const path1yml = path.join(__dirname, '__fixtures__', 'before_complex.yml');
  const path2yml = path.join(__dirname, '__fixtures__', 'after_complex.yml');

  const path1ini = path.join(__dirname, '__fixtures__', 'before_complex.ini');
  const path2ini = path.join(__dirname, '__fixtures__', 'after_complex.ini');
